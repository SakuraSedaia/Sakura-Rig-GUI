...
# =============
# region Manifest
bl_info = {  # Just a manifest containing details which can be referenced by other modules
    "author": "Your Name",
    "author_id": "Nickname",
...
class RIG1_UI1_ui_global(Panel):
    ...
    def draw(self, context):
        ...
        for l in enumerate(rig_child):
            if config_objs['materials'] in l[1].name:
                MatObj = rig_child[l[0]]
                break

        meshCol = utils.lookup_name(
            list=rig_col.children, query="Mesh")
        skin = MatObj.material_slots[0].material.node_tree
